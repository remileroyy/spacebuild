...
layout(set = 0, binding = 4, std430) restrict buffer VoxelBuffer
{
	float data[];
}
voxels;
...
void dig(vec3 coord)
{
	int index = int(coord.x + (params.numVoxelsPerAxis + 1.0) * (coord.y + (params.numVoxelsPerAxis + 1.0) * coord.z));
	...
	vec3 digCenter = vec3(params.digX, params.digY, params.digZ);
	float dist = distance(worldPos, digCenter);
	if (dist < params.digRadius)
	{
		voxels.data[index] -= params.digRadius - dist;
	}
}
...
void main()
{
	...
	if (params.digRadius > 0.1)
	{
		dig(vec3(id.x + 0, id.y + 0, id.z + 0));
	}
	else
	{
