...
{   
	...
	if (voxels[index] == 0)
	{
		vec3 chunkPos = vec3(posX, posY, posZ);
		vec3 worldPos = chunkPos + coord;
		vec3 samplePos = worldPos * scale + offset;
		float dist = length(worldPos);
		voxels[index] = 0.5 + 0.5 * noiseAt(samplePos) - 1.0 * dist/size;
	}
...
void main()
{
	...
	if (digR > 0.1)
	{
		vec3 coord = vec3(id.x, id.y, id.z);
		int index = int(coord.x + (size + 1.0) * (coord.y + (size + 1.0) * coord.z));
		vec3 chunkPos = vec3(posX, posY, posZ);
		vec3 digPos = vec3(digX, digY, digZ);
		vec3 worldPos = chunkPos + coord;
		float dist = distance(worldPos, digPos);
		if (dist < digR)
		{
